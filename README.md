...
```
Чтобы при первоз запуске grafana уже были подключены источники данных loki и готовые дашборды, необходимо создать папку **provisioning**, которую нужно смонтировать в контейнер.
