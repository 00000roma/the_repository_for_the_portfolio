# Защита веб-приложения. WAF для блокировки вредоносных запросов.  IDS/IPS на базе Suricata для обнаружения сетевых аномалий. Мониторинг на базе Promtail/Loki/Grafana.#


# ВВЕДЕНИЕ #

Данный проект сделан по примеру моей дипломной работы на тему "Проектирование и внедрение комплекса мер защиты периметра корпоративной сети. WAF для противодействия DDoS. IDS/IPS на базе Suricata и системы анализа трафика". 

Основная идея развернуть весь программный стек в контейнерах, для изучения Docker, еще можно добавить kubernetes, но это потом. Также использовать git и github, чтобы хранить весь код в удаленном репозитории и иметь доступ к нему с любого устройства, ну и для изучения git. 

Проект будет развернут на мини-ПК (достаточно слабом, но для теста подойдет):
  1) ОС: ubuntu server без графической оболочки.
  2) ОЗУ: 8гб, ddr3
  3) ССД: 125гб
  4) ЦП: intel celeron J1900, 2GHz, 4 ядра 4 потока.

Из-за ограниченных ресурсов придется контролировать контейнеры по ресурсам. Когда докер-контейнеры запускаются без ограничений ресурсов, они могут: 
1) потреблять всю доступную память хоста
2) Использовать все CPU ядря без ограничений
3) Заполнять дисковое пространство неограниченно
4) Создавать сетевые соединения без ограничений

Это приводит к:
1) нестабильной работе других контейнеров
2) зависанию системы
3) невозможности предсказать производительность
4) сложностям в масштабировании

Преимущества лимитов:
1) Изоляция ресурсов: каждый контейнер получает гарантированные ресурсы
2) Предсказуемость: поведение приложений становится предсказуемым
3) Стабильность: система остается стабильной даже при высокой нагрузке
4) Безопасность: предотвращение атак типа DoS через исчерпание ресурсов
